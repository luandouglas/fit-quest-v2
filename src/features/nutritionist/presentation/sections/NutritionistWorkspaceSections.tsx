...
type NutritionistWorkspaceSectionsProps = {
  forcedTab?: NutritionistTab;
};

export function NutritionistWorkspaceSections({
  forcedTab,
}: NutritionistWorkspaceSectionsProps) {
  ...
  const tab = forcedTab ?? resolveNutritionistTab(section);
  ...
  const [isLoadingStudentProgress, setIsLoadingStudentProgress] =
    useState(false);
  ...
  function handleChangeTab(nextTab: NutritionistTab) {
    const basePath = location.pathname.startsWith("/tabs/nutritionist")
      ? "/tabs/nutritionist"
      : "/nutritionist";
  ...
  async function handleCreateDiet() {
    ...
    const totalMacroPct =
      payload.macroDistribution.proteinPct +
      payload.macroDistribution.carbsPct +
      payload.macroDistribution.fatPct;
  ...
  async function handleReviewWeight() {
    ...
    if (
      ...
      toast({
        ...
        description:
          "Selecione um log de peso, informe valor valido e comentario tecnico.",
        tone: "warning",
      });
      return;
    }
  ...
  async function handleReviewMeasurements() {
    ...
    if (
      ...
      toast({
        title: "Dados obrigatorios",
        description:
          "Selecione um log de medidas, informe valores validos e comentario tecnico.",
        tone: "warning",
      });
      return;
    }
  ...
  async function handleGrantAchievement() {
    ...
    try {
      ...
      toast({
        ...
        description:
          "Conquista especial integrada com gamificacao e notificacoes.",
        tone: "success",
      });
    } catch (requestError) {
      toast({
  ...
  if (uiState === "error") {
    return (
      <section className="fq-page-shell">
        <FqAlert
          tone="danger"
          title="Falha ao carregar painel do nutricionista"
        >
  ...
  if (uiState === "empty" || !overview) {
    return (
      <section className="fq-page-shell">
        <FqEmptyState
          icon="utensils"
          title="Sem dados do nutricionista"
    ...
    <section className="theme-nutritionist fq-page-shell">
      ...
      {tab === "diets" ? (
        <div className="grid gap-4 lg:grid-cols-12">
          <FqCard
            className="border-border bg-card lg:col-span-7"
            title="Criar plano alimentar"
            subtitle="Refeicoes por horario + meta calorica + macros."
          >
            <div className="space-y-3">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <div className="grid grid-cols-3 gap-2">
                  <FqInput
                    ...
                    max={100}
                    value={proteinPct}
                    onChange={(event) => setProteinPct(event.target.value)}
                  />
                  <FqInput
                    label="% C"
                    type="number"
                    ...
                    max={100}
                    value={carbsPct}
                    onChange={(event) => setCarbsPct(event.target.value)}
                  />
                  <FqInput
                    label="% G"
                    type="number"
              ...
              <div className="space-y-2">
                ...
                {meals.map((meal) => (
                  <div
                    key={meal.id}
                    className="grid gap-2 rounded-xl border border-border p-3 sm:grid-cols-6"
                  >
                    <FqInput
                      label="Refeicao"
                      value={meal.name}
                    ...
                    <FqInput
                      ...
                      onChange={(event) =>
                        setMeals((current) =>
                          current.map((item) =>
                            item.id === meal.id
                              ? { ...item, calories: event.target.value }
                              : item,
                          ),
                        )
                      }
                    ...
                    <FqInput
                      ...
                      onChange={(event) =>
                        setMeals((current) =>
                          current.map((item) =>
                            item.id === meal.id
                              ? { ...item, protein: event.target.value }
                              : item,
                          ),
                        )
                      }
                ...
                <div className="flex gap-2">
                  <FqButton
                    ...
                    tone="neutral"
                    onClick={() =>
                      setMeals((current) => [...current, createMealDraft()])
                    }
                  >
                    Adicionar refeicao
                  </FqButton>
              ...
              <FqButton
                onClick={() => void handleCreateDiet()}
                isLoading={isCreatingDiet}
              >
          ...
          <FqCard
            className="border-border bg-card lg:col-span-5"
            title="Dietas vinculadas"
            subtitle="Ultimos planos criados para alunos."
          >
            <div className="space-y-2">
              {overview.diets.length === 0 ? (
                <FqText as="p" className="text-sm text-muted-foreground">
                  Nenhuma dieta vinculada ainda.
                </FqText>
              ) : (
                overview.diets.slice(0, 10).map((diet) => (
                  ...
                  >
                    ...
                    <FqText as="p" className="text-xs text-muted-foreground">
                      {diet.studentName} • {diet.dailyCalories} kcal •{" "}
                      {diet.mealsCount} refeicoes
                    </FqText>
                    <FqText as="p" className="text-xs text-muted-foreground">
                      Macros: P {diet.macroDistribution.proteinPct}% • C{" "}
                      {diet.macroDistribution.carbsPct}% • G{" "}
                      {diet.macroDistribution.fatPct}%
                    </FqText>
                  </div>
                ))
              )}
      ...
      {tab === "dashboard" || tab === "assessments" || tab === "progress" ? (
        <div className="space-y-4">
                ...
                {overview.assessments.length > 0
                  ? Math.round(
                      overview.assessments.reduce(
                        (total, item) => total + item.avgCalories7d,
                        0,
            ...
            <FqCard className="border-border bg-card">
              ...
              >
                {overview.assessments.length > 0
                  ? Math.round(
                      overview.assessments.reduce(
                        (total, item) => total + item.adherencePct7d,
                        0,
                      ) / overview.assessments.length,
                    )
                  : 0}
              ...
              >
                {
                  overview.assessments.filter((item) =>
                    item.alerts.some(
                      (alert) => alert.type === "weight-off-track",
                    ),
                  ).length
                }
              </FqText>
          ...
          <FqCard
            ...
            title="Visao nutricional por aluno"
            subtitle="Calculo automatico com consumo, dieta e progresso de peso."
          >
            {overview.assessments.length === 0 ? (
              <FqText as="p" className="text-sm text-muted-foreground">
                ...
                {overview.assessments.map((assessment) => (
                  ...
                  >
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      ...
                      <FqText as="p" className="text-xs text-muted-foreground">
                        Check-in:{" "}
                        {assessment.lastCheckinAt
                          ? new Date(assessment.lastCheckinAt).toLocaleString(
                              "pt-BR",
                            )
                          : "Sem registros"}
                      </FqText>
                    ...
                    <div className="mt-1 grid gap-1 sm:grid-cols-2 lg:grid-cols-4">
                      <FqText as="p" className="text-xs text-muted-foreground">
                        Consumo medio: {assessment.avgCalories7d} kcal (meta{" "}
                        {assessment.calorieTargetDaily} kcal)
                      </FqText>
                      <FqText as="p" className="text-xs text-muted-foreground">
                        Aderencia dieta: {assessment.adherencePct7d}% •
                        Calorias: {assessment.calorieAdherencePct7d}%
                      </FqText>
                      <FqText as="p" className="text-xs text-muted-foreground">
                        Peso atual:{" "}
                        {assessment.weightCurrentKg !== null
                          ? `${assessment.weightCurrentKg.toFixed(1)} kg`
                          : "Sem dado"}
                      </FqText>
                      <FqText as="p" className="text-xs text-muted-foreground">
                        Evolucao 30d:{" "}
                        {assessment.weightDeltaKg30d !== null
                          ? `${assessment.weightDeltaKg30d > 0 ? "+" : ""}${assessment.weightDeltaKg30d} kg`
                          : "Sem dado"}
                      </FqText>
                    </div>
                    <FqText
                    ...
                    >
                      Projecao:{" "}
                      {assessment.projectedWeightKg !== null
                        ? `${assessment.projectedWeightKg.toFixed(1)} kg`
                        : "Sem projecao"}{" "}
                      (
                      {assessment.projectionDeltaKg !== null
                        ? `${assessment.projectionDeltaKg > 0 ? "+" : ""}${assessment.projectionDeltaKg} kg`
                        : "n/a"}
                      )
                    </FqText>
                      ...
                      <div className="mt-2 space-y-1">
                        {assessment.alerts.map((alert) => (
                          <FqText
                            key={alert.id}
                            as="p"
                            className={`text-xs ${alert.severity === "danger" ? "text-rose-500" : "text-amber-500"}`}
                          >
                            {alert.type === "goal-not-hit"
                              ? "Meta nao batida:"
                              : "Peso fora da projecao:"}{" "}
                            {alert.message}
                          </FqText>
                        ))}
                      </div>
                    ) : (
                      <FqText as="p" className="mt-2 text-xs text-emerald-500">
                        Sem alertas para este aluno.
                      </FqText>
                    )}
                  </div>
                ))}
          ...
          <FqCard
            ...
            subtitle="Requests de avaliacao enviados pelos alunos."
          >
          ...
          <div className="grid gap-4 lg:grid-cols-12">
            <FqCard
              className="border-border bg-card lg:col-span-5"
              title="Revisao profissional"
              subtitle="Peso e medidas com trilha auditavel."
            >
              <div className="space-y-3">
                <FqSelect
                  label="Aluno"
                ...
                <FqSelect
                  ...
                  onChange={(event) =>
                    setSelectedWeightLogId(event.target.value)
                  }
                  options={(studentProgress?.weightLogs ?? [])
                    .slice(0, 10)
                    .map((log) => ({
                      value: log.id,
                ...
                />
                <FqInput
                  label="Peso ajustado (kg)"
                  value={reviewWeightKg}
                  onChange={(event) => setReviewWeightKg(event.target.value)}
                />
                ...
                <FqSelect
                  ...
                  onChange={(event) =>
                    setSelectedMeasurementLogId(event.target.value)
                  }
                  options={(studentProgress?.bodyMeasurementLogs ?? [])
                    .slice(0, 10)
                    .map((log) => ({
                      value: log.id,
                ...
                <div className="grid grid-cols-2 gap-2">
                  <FqInput
                    label="Peito"
                    value={reviewMeasurements.chestCm}
                    onChange={(event) =>
                      setReviewMeasurements((current) => ({
                        ...current,
                  ...
                  <FqInput
                    label="Cintura"
                    value={reviewMeasurements.waistCm}
                    onChange={(event) =>
                      setReviewMeasurements((current) => ({
                        ...current,
                  ...
                  <FqInput
                    label="Quadril"
                    value={reviewMeasurements.hipsCm}
                    onChange={(event) =>
                      setReviewMeasurements((current) => ({
                        ...current,
                  ...
                  <FqInput
                    label="Braco"
                    value={reviewMeasurements.armCm}
                    onChange={(event) =>
                      setReviewMeasurements((current) => ({
                        ...current,
                        armCm: event.target.value,
                      }))
                    }
                  />
                  <FqInput
                    label="Coxa"
                    value={reviewMeasurements.thighCm}
                    onChange={(event) =>
                      setReviewMeasurements((current) => ({
                        ...current,
                ...
                <div className="flex flex-wrap gap-2">
                  <FqButton
                    onClick={() => void handleReviewWeight()}
                    isLoading={isReviewingWeight}
                  >
                  ...
                  <FqButton
                    variant="outline"
                    tone="secondary"
                    onClick={() => void handleReviewMeasurements()}
                    isLoading={isReviewingMeasurements}
                  >
            ...
            <FqCard
              className="border-border bg-card lg:col-span-7"
              title="Historico imutavel"
              subtitle="Cada ajuste gera nova entrada com referencia ao log original."
            >
              {isLoadingStudentProgress ? (
                ...
                <div className="space-y-2">
                    ...
                    .map((log) => (
                      ...
                      >
                        <FqText
                          ...
                          className="text-sm font-semibold text-foreground"
                        >
                          Peso {log.weightKg.toFixed(1)}kg •{" "}
                          {log.recordedByRole}
                        </FqText>
                        <FqText
                          as="p"
                        ...
                        >
                          ...
                          {log.revisedFromLogId
                            ? ` • revisao de ${log.revisedFromLogId}`
                            : ""}
                        </FqText>
                        {log.comment ? (
                          <FqText
                  ...
                  {(studentProgress?.bodyMeasurementLogs ?? [])
                    .slice(0, 8)
                    .map((log) => (
                      <div
                        key={log.id}
                      ...
                      >
                        <FqText
                          ...
                          className="text-sm font-semibold text-foreground"
                        >
                          Medidas • cintura {log.measurements.waistCm}cm • peito{" "}
                          {log.measurements.chestCm}cm
                        </FqText>
                        <FqText
                          as="p"
                          className="text-xs text-muted-foreground"
