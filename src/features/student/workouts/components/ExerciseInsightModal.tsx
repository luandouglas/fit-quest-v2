...
export function ExerciseInsightModal({
  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 z-50 bg-[rgba(30,40,36,0.52)] backdrop-blur-md" />
        <Dialog.Content
          className="fq-gradient-surface fixed left-1/2 top-1/2 z-[60] flex h-[min(88vh,860px)] w-[min(760px,94vw)] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-[32px] border border-border/70 bg-card shadow-[0_36px_100px_rgba(34,49,43,0.24)] focus:outline-none"
        >
            ...
            <div className="border-b border-border/70 px-5 py-5 md:px-6 md:py-6">
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <Dialog.Description className="mt-2 text-sm leading-relaxed text-muted-foreground">
                      Tudo o que voce precisa para executar com mais confianca, ritmo e consistencia.
                    </Dialog.Description>
                  </div>
                ...
                <Dialog.Close asChild>
                  <button
                    type="button"
                    className="inline-flex h-11 w-11 items-center justify-center rounded-full border border-border/70 bg-white/80 text-muted-foreground transition hover:border-border hover:bg-white hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                    aria-label="Fechar detalhes do exercicio"
                  >
                    <X className="h-4 w-4" />
                  </button>
                </Dialog.Close>
